...
def sha1_split_blocks(message):
    
    return [message[i:i+64] for i in range(0, len(message), 64)]


def sha1_generate_W(block):
...
def sha1_compression(block, H):
    """Fonction de compression SHA-1."""
    W = sha1_generate_W(block)
    A, B, C, D, E = H
...
def sha256_split_blocks(message):
    """Découpe le message en blocs de 64 octets."""
    return [message[i:i+64] for i in range(0, len(message), 64)]


def sha256_generate_W(block):
    """Génère les mots W[0..63] pour SHA-256."""
    W = []
    for i in range(16):
        W.append(int.from_bytes(block[i*4:(i*4)+4], 'big'))
...
def sha256_compression(block, H):
    """Fonction de compression SHA-256."""
    # Constantes K[0..63] : racines cubiques des 64 premiers nombres premiers
    K = [
...
def sha256(message):
    """
    Implémentation pure de SHA-256.
    Retourne le hash hexadécimal.
    """
    H = [
    ...
    blocks = sha256_split_blocks(message)
...
# =============================================================================
# PARTIE 0.5: FONCTIONS RSA POUR CHAÎNES
# =============================================================================
...
def rsa_generer_pq():
    ...
    n = np_random.randint(500, 1000)
    m = np_random.randint(500, 1000)
    while not est_premier(n):
        n += 1
    while not est_premier(m) or m == n:
        m += 1
    return n, m


def rsa_generer_cles_simple():
    """Génère une paire de clés RSA simples."""
    p, q = rsa_generer_pq()
    n = p * q
    phi = (p - 1) * (q - 1)
    e = 3
...
def rsa_chiffrer_texte(texte, pub_key):
    """Chiffre un texte caractère par caractère avec RSA."""
    e, n = pub_key
    chiffres = []
    for c in texte:
        m = ord(c)
        if m >= n:
            raise ValueError(f"Caractère '{c}' (valeur {m}) ≥ n={n}. Choisissez des nombres premiers plus grands.")
        chiffres.append(pow(m, e, n))
    return ','.join(str(c) for c in chiffres)


def rsa_dechiffrer_texte(chiffres_str, priv_key):
    """Déchiffre un texte RSA caractère par caractère."""
    d, n = priv_key
    chiffres = [int(x) for x in chiffres_str.split(',')]
    return ''.join(chr(pow(c, d, n)) for c in chiffres)
...
def vernam_chiffrer(texte: str, cle: str) -> tuple:
    ...
    # Si la clé est vide, on en génère une aléatoire
    if len(cle_bytes) == 0:
        cle_bytes = bytes([random.randint(0, 255) for _ in range(len(msg_bytes))])
    
    # Si la clé est plus courte, on la répète 
...
def des_str_to_bits(s: str, longueur: int) -> list:
    """Convertit une chaîne en liste de bits (padding si nécessaire)."""
    bits = []
    s = s[:longueur]
    for c in s:
...
def des_chiffrer_bloc(bloc_64: list, sous_cles: list) -> list:
    ...
    # Échange final (L et R sont échangés avant IP^-1)
    bloc_final = R + L
    return des_permuter(bloc_final, IP_INV)


def des_dechiffrer_bloc(bloc_64: list, sous_cles_inv: list) -> list:
    """Déchiffre un bloc de 64 bits avec DES (reçoit les sous-clés en ordre inverse)."""
    bloc = des_permuter(bloc_64, IP)
    ...
    for i in range(16):
        L_suivant = R
        F = des_fonction_f(R, sous_cles_inv[i])
        R_suivant = [L[j] ^ F[j] for j in range(32)]
        L, R = L_suivant, R_suivant
...
# DES MODE ECB (ELECTRONIC CODEBOOK)
...
def des_dechiffrer(chiffre_hex: str, cle: str) -> str:
    ...
    for i in range(0, len(chiffre_bytes), 8):
        ...
        # Déchiffrement
        bloc_dechiffre_bits = des_dechiffrer_bloc(bloc_bits, sous_cles_inverse)
...
def cfb_chiffrer(texte: str, cle: str, iv_hex: str = None) -> dict:
    ...
    for i in range(0, len(texte_bytes), 8):
        ...
        while len(bloc_chiffre) < 8:
            bloc_chiffre.append(0)
        
        resultat.extend(bloc_chiffre[:len(bloc_clair)])
        registre = list(bloc_chiffre)
...
def cfb_dechiffrer(chiffre_hex: str, cle: str, iv_hex: str) -> str:
    ...
    for i in range(0, len(chiffre_bytes), 8):
        ...
        # Chiffrement du registre (identique au chiffrement)
        registre_bits = []
        for octet in registre:
            registre_bits.extend([int(b) for b in format(octet, '08b')])
        ...
        registre = list(bloc_chiffre + bytes(8 - len(bloc_chiffre)))[:8]
