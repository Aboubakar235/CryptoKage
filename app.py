...
from crypto_algorithms import (
    ...
    sha1, sha256  # Import des nouvelles fonctions de hachage
)
...
def init_db():
    ...
    if cursor.fetchone()[0] == 0:
        # Mot de passe: Admin@1234 haché avec notre SHA-256 implémenté en pur Python
        mdp_hash = sha256('Admin@1234')
        cursor.execute('''
        ...
        print(f"[INFO] Hash du mot de passe admin: {mdp_hash}")
...
def hacher_mot_de_passe(mdp: str) -> str:
    """Hache un mot de passe avec notre implémentation SHA-256."""
    return sha256(mdp)
...
def register():
    ...
    if request.method == 'POST':
        ...
        # Hachage et insertion avec notre SHA-256
...
@admin_requis
def admin_dashboard():
    """Dashboard administrateur avec affichage des hashs SHA-256."""
    conn = get_db()
    utilisateurs = conn.execute(
        "SELECT id, username, email, password, role, created_at FROM utilisateurs ORDER BY id"
    ).fetchall()
    conn.close()
...
def api_crypto():
    ...
    try:
        ...
        # SHA-1 (hash)
            ...
            resultat = sha1(message)
            return jsonify({'resultat': resultat})
        
        # SHA-256 (hash)
        elif algo == 'sha256':
            resultat = sha256(message)
            return jsonify({'resultat': resultat})
...
def changer_mot_de_passe():
    ...
    if not user or not verifier_mot_de_passe(ancien_mdp, user['password']):
...
# Au début, modifier la ligne du secret_key (optionnel)
app.secret_key = os.environ.get('SECRET_KEY', 'cryptokage_secret_key_2024')

# À la fin, modifier le message de démarrage :
if __name__ == '__main__':
    init_db()
    print("=" * 50)
    print("CryptoKage - Application de Cryptographie")
